...
namespace EtmsSystem.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class ETMSController : ControllerBase
    {
        ...
        public List<TeamMember> team( int pid)
        {
            ...
            using (var db = new EtmsystemContext())
            {
               ...
               List< TeamMember> team = db.TeamMembers.Include(e => e.Emp).ThenInclude(e => e.Desig).Include(e => e.Role).Where(e=>e.ProjectId == pid).ToList();
                return team;
            }
